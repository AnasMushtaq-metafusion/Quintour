...
const TourVideoScreen = ({ navigation, route }: any) => {
  ...
  // console.log(data, id, nodes, edges);
  ...
  //     // console.log('progress -->' + Math.floor(progress.currentTime*width / 100));
      ...
      <LayoutOverlay data={data} itemDuration={itemDuration}>
        ...
        <View style={styles.container}>
          <View style={styles.progress}>
            {/* <View style={{width: `${currentTime/duration*100}%`, height: '100%', backgroundColor: 'red'}}></View> */}
            {/* <Text>
                        {
                            const fileName = '480af9a2f2d2430b900cad3ec8b0a40e.mp4'; // Replace with your desired file name
                            const downloadDest = `${RNFS.DocumentDirectoryPath}/i_tour/${itemId}/${fileName}`; 
                        }
                    </Text> */}
            <LinearGradient
          ...
          {data?.url ? (
            <>
              <Video
                // ref={videoRef}
                // source={itemId && require(`${RNFS.DocumentDirectoryPath}/i_tour/${itemId}/${fileName}`)}
